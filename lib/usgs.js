...
export async function fetchLiveReadings(gaugeIds) {
  ...
  const url = `${USGS_IV}?format=json&sites=${sites}&parameterCd=00060,00010,00065&siteStatus=active`;
  try {
    ...
    // Consider a reading stale if older than 48 hours
    ...
    for (const s of series) {
      ...
      const value    = (raw === null || raw === -999999) ? null : raw;
      const dt       = latest?.dateTime || null;

      if (!readings[siteId]) readings[siteId] = { siteId, siteName, flow: null, temp_c: null, gage: null, timestamp: null };

      // Flow is the most reliable — always use it if available
      if (param === '00060') {
        readings[siteId].flow = value;
        if (dt) readings[siteId].timestamp = dt;
      }
      // Temp and gage: only use if the reading is fresh (sensor could be offline)
      if (param === '00010') {
        readings[siteId].temp_c = isFresh(dt) ? value : null;
        if (!isFresh(dt) && value !== null) {
          console.log(`[usgs] Stale temp for ${siteId}: ${dt} (${value}°C) — discarding`);
        }
      }
      if (param === '00065') {
