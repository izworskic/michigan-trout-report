...
export async function fetchLiveReadings(gaugeIds) {
  ...
  try {
    ...
    for (const s of series) {
      ...
      // Flow — also check freshness (gauges do go offline)
      if (param === '00060') {
        readings[siteId].flow = isFresh(dt) ? value : null;
        if (dt) readings[siteId].timestamp = dt;
        if (!isFresh(dt) && value !== null) console.log(`[usgs] stale flow ${siteId} (${dt?.slice(0,10)})`);
      }
      // Temp, gage, turbidity, DO — only if fresh (sensors go offline seasonally)
      if (param === '00010') {
      ...
      if (param === '00065') {
        ...
        if (!isFresh(dt) && value !== null) console.log(`[usgs] stale gage ${siteId} (${dt?.slice(0,10)})`);
      }
      if (param === '63680') {
  ...
  try {
    ...
    for (const line of text.split('\n')) {
      ...
      if (parts[5] === mo && parts[6] === dy) {
        return {
          gaugeId,
          p10: parseFloat(parts[10]) || null,
          p25: parseFloat(parts[11]) || null,
          p50: parseFloat(parts[12]) || null,
          p75: parseFloat(parts[13]) || null,
          p90: parseFloat(parts[14]) || null,
        };
      }
    }
